...
function EditRepaIntervention({
  ...
  const addToList = () => {
    if (repaAfaire.length > 0) {
      ...
      let data = repaAfaire.filter(
        (item) => item.actionName !== values["actionName"]
      );
      setRepaAfaire([
        ...data,
        { id: uuidv4(), actionName: values["actionName"] },
      ]);
    } else {
      setRepaAfaire([
        ...repaAfaire,
  ...
  return (
    <>
      ...
      <form className="form">
        <h1 className="titleForm">Actions / Reparations effectuees </h1>
   ...
   <input name='commande' type='checkbox' value={isCommande} onChange={()=>setIsCommande(!isCommande)}/>
        ...
        <div className="inputRowContainer">
          ...
          <div className="formInput">
            <label className="labelInput">{"Selectionnez action"}</label>
            <input
            ...
            {repaAfaire.map((repa, index) => (
              <tr key={index} className="active-row">
                ...
                <td>
                  <span
                    className="delItem"
                    onClick={() => removeItemListeRepa(repa)}
                  >
                    {" "}
                    <DeleteIcon />
                  </span>
...
export default EditRepaIntervention;

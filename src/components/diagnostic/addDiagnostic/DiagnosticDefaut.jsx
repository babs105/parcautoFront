...
function DiagnosticDefaut({defauts,focused , setFocused, nextStep ,prevStep,values, onChange, setDefautsConsta,defautsConsta,removeItemListeDefaut}) {
    ...
    const Continue = e => {
      ...
      const addToList =()=>{
       ...
       console.log('tab not empty',  defautsConsta.length )
        let data = defautsConsta.filter(item =>item.defautName !== values['defautName'] 
       )   
       setDefautsConsta([...data,{ id: uuidv4(),defautName:values['defautName']} ])
      }else{
    ...
    return (
                    ...
                    value={values['statut'] }   
                      >
                         <option key={1} value={""}>
                          ...
                          <option key={2} value={"defaillance"}>
                            {'defaillance'}
                          </option>
                          <option key={3} value={"panne"}>
                            {"panne"}
                          </option>
                      </select>
      ...
      <span className='inputError'>{"Defaut Obligatoire"}</span>
    ...
    {defautsConsta.map((defaut ,index) =>
        <tr  key={index} className="active-row">
            ...
            <td><span className='delItem' onClick={() =>removeItemListeDefaut(defaut)}> <DeleteIcon/></span></td>
        </tr>
       )
      }   </tbody>
      </table>
...
export default DiagnosticDefaut
